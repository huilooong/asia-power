...
import { turnPolicy, routedReply } from "./apsales-turn-policy.mjs";
...
async function runOpenClawReply({ text, senderId, messageId, chatId, observedAt, mediaPlaceholder, mediaContext, dealState, inventoryMatches, approximateMatches }) {
  ...
  const prompt = [
    ...
    "- Answer the customer's CURRENT question first (price, Ghana vs China stock, shipping days, payment, manual vs automatic). Only AFTER answering may you ask ONE missing closing detail (quantity / port / payment / ETA acceptance). Never ignore a direct commercial question.",
    "- If last_outbound_cue is offer_price_go_ahead and the customer affirms (yes / ok / sure / please / go ahead — judge meaning), that is a go-ahead toward pricing: do NOT stall or re-ask the same VIN. Ask the ONE still-missing qualify detail (prefer engine code when year+model are known), or if you already have enough to check stock, say you will price it — keep reasoning, no scripted filler.",
    "- If vin_unusable is true (team said this VIN/number is not standard / cannot decode), do NOT keep asking for that same VIN or \"another VIN\" in a loop. Switch to engine code / clear metal plate photo, and use year+model+fuel/engine family (e.g. CRD) as working assumptions while marking uncertainty. Still never invent a firm price number without evidence.",
    "- Set needs_price_confirmation to true ONLY when this reply tells the customer a price/stock still needs team confirmation. NEVER set it true just for acknowledging a VIN/photo or asking for engine code — that wrongly silences the customer.",
    "- If recent_team_replies is non-empty, a human teammate already messaged this customer directly — those texts are authoritative facts for this thread. If they already answer the customer's current question (price, import/China, ETA, payment), USE that answer in customer_reply. Do NOT contradict, deny, re-ask what they answered, or stall with \"still checking\" / \"what do you need next?\"; only add new info or ask one still-missing closing detail.",
    ...
    "- Technical facts (engine codes, model/engine families, part fitment/compatibility, general specs) are public knowledge — reason about them yourself even without an exact inventory_matches hit, the same way you would if the customer asked you directly with no tools. Business-private facts (our actual price, our actual stock count, our actual ETA) are different — never invent a specific number for those without real evidence.",
    "- If inventory_matches has no exact hit but the customer's engine/model is clearly in a family or category you can recognize, say so plainly and ask the one detail that would pin down the exact match (e.g. exact engine code, year, or VIN) — do not default to \"the team will confirm\" just because there is no exact row. Only say the price/stock needs team confirmation, and set needs_price_confirmation to true, when you genuinely have no reasonable basis to say anything more useful than that.",
    ...
    "- Soft chat angles (5W2H material — NOT a checklist, NOT gated on possible_repeat_detected): uncovered_closing_angles is background context every turn, not a special-case trigger. Use your own judgment on whether this turn is a natural moment to weave in AT MOST ONE angle (why they need it, how urgent/when, destination port/where, payment preference/how, or quantity/how_much) to keep the conversation moving toward a sale. possible_repeat_detected being true is a strong hint you are stuck in a holding loop and should lean toward picking one — it is a signal, not the only trigger. Skip entirely if none fits this turn, or if the customer shows an exit signal (later/wait/off-topic/does not want to expand). Still max one question total per turn.",
    softAngleDryRun
      ...
      : "- soft_angle_dry_run is false: after answering the customer, you MAY weave at most ONE uncovered_closing_angles item into customer_reply when this turn is a natural moment to advance the deal. possible_repeat_detected being true is a strong hint to pick one — it is NOT required. Set chat_angle_used to that angle id (why|when|where|how|how_much) or \"\" if you skipped.",
    ...
    JSON.stringify({
      ...
      vin_unusable: Boolean(
        (Array.isArray(dealState?.team_replies) ? dealState.team_replies : []).some((r) =>
          /not a standard vin|invalid vin|can'?t get information through this vin|cannot get information through this vin|vin.*(not|no).*valid/i.test(
            String(r?.text || ""),
          ),
        ),
      ),
      last_outbound_cue: String(dealState?.last_outbound_cue || "") || null,
