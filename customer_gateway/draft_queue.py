...
def save_draft(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    record = {
        ...
        "authority": payload.get("authority", "sales"),
        "constitution_allowed": payload.get("constitution_allowed", True),
        "constitution_reason": payload.get("constitution_reason", ""),
        "constitution_rule": payload.get("constitution_rule", ""),
        ...
        "inventory_confidence": payload.get("inventory_confidence"),
...
def format_draft_detail(draft: dict[str, Any]) -> str:
    lines = [
        ...
        f"权限 authority: {draft.get('authority', 'sales')}",
        f"宪章规则: {draft.get('constitution_rule', '')}",
        f"权限检查: {draft.get('authority_check', '')}",
        f"风险分: {draft.get('risk_score', 0)}",
        f"风险原因: {draft.get('risk_reason', '')}",
        f"创建: {draft.get('created_at')}",
        "",
        "—— 决策路径 ——",
        draft.get("decision_path", ""),
        "",
        "—— 推理摘要 ——",
