...
for (const file of ['server/half-cut-local-server.js', 'deploy/inventory-site-server.js']) {
  test(`${file}: APBD handlers enforce auth, return data, and rate-limit work`, async () => {
    const source = fs.readFileSync(path.join(__dirname, '..', file), 'utf8');
    const start = source.indexOf("      if (req.method === 'GET' && p === '/api/admin/apbd/solo-trade')");
    const end = source.indexOf('\n      if (', source.indexOf("return json(res, err.statusCode || 400", start));
    assert.ok(start >= 0 && end > start);
    const handler = new Function('ctx', `with (ctx) { return (async () => { ${source.slice(start, end)} })(); }`);
    let allowed = false;
    let rateAllowed = true;
    let reads = 0;
    let starts = 0;
    const guard = (_req, res) => { if (!allowed) res.code = 401; return allowed; };
    const ctx = {
      req: {}, res: {}, p: '', ROOT: '/fixture', ASIAPOWER_ROOT: '/fixture', MAX_AUTH_BODY: 65536,
      requireAdmin: guard, auth: { requireAdmin: guard },
      json: (res, code, body) => Object.assign(res, { code, body }),
      buildPromotionSnapshot: () => { reads++; return { summary: { totalLeads: 2 } }; },
      nativeEnrichment: { status: () => { reads++; return { status: 'idle' }; }, start: () => { starts++; return { status: 'running' }; } },
      limitNativeEnrichment: () => rateAllowed, readBody: async () => ({}),
    };
    for (const [method, suffix] of [['GET', 'solo-trade'], ['GET', 'native-enrichment/status'], ['POST', 'native-enrichment/run']]) {
      ctx.req.method = method; ctx.p = `/api/admin/apbd/${suffix}`; ctx.res = {};
      await handler(ctx);
      assert.equal(ctx.res.code, 401);
    }
    assert.equal(reads, 0); assert.equal(starts, 0);
    allowed = true;
    ctx.req.method = 'GET'; ctx.p = '/api/admin/apbd/solo-trade'; ctx.res = {};
    await handler(ctx); assert.equal(ctx.res.code, 200); assert.equal(ctx.res.body.summary.totalLeads, 2);
    ctx.p = '/api/admin/apbd/native-enrichment/status'; ctx.res = {};
    await handler(ctx); assert.equal(ctx.res.code, 200); assert.equal(ctx.res.body.status, 'idle');
    ctx.req.method = 'POST'; ctx.p = '/api/admin/apbd/native-enrichment/run'; rateAllowed = false; ctx.res = {};
    await handler(ctx); assert.equal(ctx.res.code, 429); assert.equal(starts, 0);
    rateAllowed = true; ctx.res = {};
    await handler(ctx); assert.equal(ctx.res.code, 202); assert.equal(starts, 1);
  });
}
