...
assert(sw.includes("CACHE_VERSION = 'pwa-app-v6c'"), 'CACHE_VERSION is pwa-app-v6c');
assert(sw.includes('isCatalogDetailNavigation'), 'detail navigations skip SW HTML cache');
...
const current = 'pwa-app-v6c-static';
const observed = [
...
assert(toDelete.length === 6, 'deletes 6 obsolete buckets from Claude evidence set');
assert(!toDelete.includes(current), 'keeps current pwa-app-v6c-static');
assert(toDelete.includes('pwa-app-v6b-static'), 'deletes prior pwa-app-v6b-static');
...
assert(installJs.includes("SW_URL = '/sw.js?v=pwa-app-v6c'"), 'pwa-install registers sw.js?v=pwa-app-v6c');
