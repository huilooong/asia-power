...
assert(indexHtml.includes('pwa-app-v6c') && indexHtml.includes('pwa-app-shell'), 'homepage wires current app shell assets');
assert(!indexHtml.includes('pwa-install.js') && !indexHtml.includes('pwa-install.css'), 'homepage omits the add-to-home-screen prompt');
...
assert(manifest.display === 'browser' && manifest.display_override?.[0] === 'browser', 'manifest keeps production browser mode');
...
async function runDomTests() {
  ...
  assert(!!shellApi && shellApi.enabled() === false, 'app shell production kill switch remains disabled');
  assert(shellApi.isStandalone() === false, 'disabled app shell ignores ?app=1 preview mode');
  assert(shellApi.applyShell() === false, 'disabled app shell does not alter the page');
  assert(!env3.document.body.classList.contains('ap-app-shell'), 'body stays outside app shell mode');
