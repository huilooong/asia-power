...
Restore the Release Manager snapshots for the `brand-registry-api` and `visual-v1` release IDs, or redeploy the prior Git commit through the same targets. Original customer media and inventory records do not require restoration because they are not changed.
